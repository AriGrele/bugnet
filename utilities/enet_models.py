...
'''
NB this is mostly the same code as in training_pipeline.image_models.Efficientnet.enen_models.
Differences have creeped in organically due to different needs in training vs deployment, so -
This script should function for for all deployment methods in database_pipeline and processing folders
training_pipeline.image_models.Efficientnet.enen_models should function for training models as described in the publication
If a model trained using that script fails to deploy properly in these folders, check that there is no new code here missing from the training script, and vice versa.
most comments have not been transfered over - check other script
'''
...
class E_model(nn.Module):
    ...
    def forward(self,x):
        if hasattr(self.model,'latent'):
            p=self.model(x)
            x=self.model.final(p)
